...
class chemiotaxis:
    ...
    def chemio(self, bacteria, poblacion, d_attr, w_attr, h_rep, w_rep):
        bacteria.interaction = self.attract_repel(
            bacteria, poblacion, d_attr, w_attr, h_rep, w_rep
        )
    ...
    def eliminarClonar(self, path, poblacion):
        puntuaciones = [bacteria.fitness for bacteria in poblacion]
        
        nuevas_bacterias, nuevas_puntuaciones = mm.reproduccion(poblacion, puntuaciones)
        
        poblacion[:] = nuevas_bacterias  # Reemplaza la población actual con las mejores y sus clones
    ...
    def clonacion(self, path, poblacion):
        ...
        for bacteria in poblacion:
            ...
            mutacion = int(
                (best.fitness - bacteria.fitness) / 10
            )  # mutacion en funcion de la diferencia de fitness
